...
local function Update(self, event, unit)
	...
	* lossPerc - the percent by which the unit's max health has been temporarily reduced (number)
	...
	if(element.PostUpdate) then
		element:PostUpdate(self, unit, cur, max, lossPerc)
	end
end
...
local function Enable(self)
	...
	if(element) then
        ...
        self:RegisterEvent('UNIT_THREAT_SITUATION_UPDATE', ColorPath)
        self:RegisterEvent('UNIT_CLASSIFICATION_CHANGED', ColorPath)
...
local function Disable(self)
	...
	if(element) then
		...
		self:UnregisterEvent('UNIT_CLASSIFICATION_CHANGED', ColorPath)
